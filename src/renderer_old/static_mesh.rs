...
type MaterialHandle = MarkedHandle<Material>;

struct MaterialPool {
    materials: HandledStdVec<Material>,
}

impl MaterialPool {
    pub fn new() -> Self {
        MaterialPool {
            materials: HandledStdVec::new()
        }
    }

    pub fn add_material(&mut self, material: Material) -> MaterialHandle {
        self.materials.add_entry(material)
    }
}

struct RenderObjectT {
    material: Material,
    geometry: Geometry,
}
